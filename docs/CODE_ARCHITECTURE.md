...
4. Deny rules inspect the complete command and its decomposed Bash leaves.
5. The command-chain limit rejects inputs with more than the configured number
   of leaves.
6. Complete allow matches return allow; unmatched calls return passthrough.
   Deny emits a reason, while allow and passthrough produce no standard output.
7. Every intercepted Codex event is written to the configured bounded audit
